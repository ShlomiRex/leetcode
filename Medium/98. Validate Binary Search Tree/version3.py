...
def isValidBST(root: Optional[TreeNode]) -> bool:
	if root == None:
		return True
	if isValidBSTHelper(root.left, float("-inf"), root.val) == False or isValidBSTHelper(root.right, root.val, float("inf")) == False:
		return False
	return True
...
def isValidBSTHelper(root: Optional[TreeNode], left_range, right_range) -> bool:
	...
	if root.val >= right_range or root.val <= left_range:
		return False
	if isValidBSTHelper(root.left, left_range, root.val) == False or isValidBSTHelper(root.right, root.val, right_range) == False:
		return False
	return True
